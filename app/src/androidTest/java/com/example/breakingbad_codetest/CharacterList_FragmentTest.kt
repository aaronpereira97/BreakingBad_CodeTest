...
import com.example.breakingbad_codetest.util.ToastMatcher
...
class CharacterList_FragmentTest : KoinTest {
    ...
    @Test
    fun test_isNotProgressBarShowingWhenFailRequest() {
        val msgError  = "error in the request"
        launchActivity()
        emitLoadingState(LoadingState.error(msgError))
        emitAllItemSearch(FakeObjectsUtils.listDBDatabaseCharacter)
        emitAllItemsFiltered(FakeObjectsUtils.listDBDatabaseCharacter)

        onView(withText(msgError))
            .inRoot(ToastMatcher())
